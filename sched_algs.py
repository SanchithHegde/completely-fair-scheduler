...
""" Definitions for scheduling algorithms like First Come First Served (FCFS)
    scheduling, Shortest Job First (preemptive) / Shortest Remaining Time First
    scheduling, Priority Scheduling (preemptive) and Round Robin Scheduling
    (preemptive). """
...
def fcfs_schedule(tasks: List[dict]):
    """ Schedule tasks according to FCFS algorithm and set waiting and
        turnaround times. """
...
def _schedule_key(tasks: List[dict], quantum: int,
                  key: Callable[[dict], Tuple[int, int, int]]):
    """ Schedule tasks according to algorithm determined byt the key and set
...
def rr_schedule(tasks: List[dict], quantum: int):
    """ Schedule tasks according to preemptive Round Robin algorithm and set
        waiting and turnaround times. """
...
def sjf_schedule(tasks: List[dict], quantum: int):
    """ Schedule tasks according to preemptive SJF algorithm and set waiting
        and turnaround times. """
    get_remaining_exec_time: Callable[[dict], Tuple[int, int, int]] = \
        lambda task: (task['burst_time'] - task['exec_time'],
                      task['arrival_time'], tasks.index(task))

    _schedule_key(tasks, quantum, get_remaining_exec_time)


def priority_schedule(tasks: List[dict], quantum: int):
    """ Schedule tasks according to preemptive priority scheduling algorithm
        and set waiting and turnaround times. """
    get_nice: Callable[[dict], Tuple[int, int, int]] = \
        lambda task: (task['nice'], task['arrival_time'], tasks.index(task))
